...
namespace OmronPlc
{
	...
	bool tcpFinsCommand::NodeAddressDataSend()
	{
		...
		// send NADS command
		//
		transport->PLCSend(cmdNADS, sizeof(cmdNADS));

		// wait for a plc response
		//
		uint8_t respNADS[24] = { 0 };
		transport->PLCReceive(respNADS, sizeof(respNADS));
	...
	bool tcpFinsCommand::FrameSend(const vector<uint8_t> &data)
	{
		...
		// send frame header
		//
		transport->PLCSend(&cmdFS[0], cmdFS.size());

		// send FINS command
		//
		transport->PLCSend(&cmdFins[0], finsCommandLen);

		// send additional data
		//
		if (data.size() > 0)
		{
			transport->PLCSend(&data[0], data.size());
		}

		// frame response
		//
		transport->PLCReceive(&respFS[0], respFS.size());
		...
		//
		transport->PLCReceive(&respFins[0], 14);
		...
		if (finsResponseLen > 14)
		{
			...
			//
			transport->PLCReceive(&respFinsData[0], finsResponseLen - 14);
		}
